...
const ContraVoucher = () => {
    ...
    const filteredContraVouchers = contraVouchers.filter(row => {
        // Date range filter
        if (row.date) {
        ...
        if (searchTerm) {
            ...
            const sourceAcc = String(row.sourceAccount?.name || '').toLowerCase();
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (coaRes.success) {
                ...
                // Show more accounts but filtered
    ...
    // Handle deep-linking of a specific contra voucher
    useEffect(() => {
        if (location.state && location.state.targetContraId && contraVouchers.length > 0) {
            const targetId = parseInt(location.state.targetContraId);
            const contraObj = contraVouchers.find(c => c.id === targetId);
            if (contraObj) {
                handleView(contraObj);
            }
            // Clear state so re-renders don't re-trigger modal opening
            navigate(location.pathname, { replace: true, state: {} });
        }
    }, [location.state, contraVouchers, navigate]);
    ...
    const handlePrint = () => {
        ...
        const itemsHtml = selectedContra.items.map(item => `
            <tr>
        ...
        const content = `
            <html>
                <head>
                    <title>Contra Voucher - ${selectedContra.voucherNumber}</title>
                    <style>
                ...
                <body>
                    <div class="header">
                        ...
                        <div style="text-align: right;">
                            <div style="font-size: 18px; font-weight: bold; color: #1a5f7a;">${getDocumentTitle('contravoucher')}</div>
                            <div style="font-family: monospace; font-size: 12px; margin-top: 5px;">NO: ${selectedContra.voucherNumber}</div>
                            <div style="font-size: 11px; color: #666; margin-top: 3px;">Date: ${new Date(selectedContra.date).toLocaleDateString()}</div>
                        </div>
                    </div>

                    <div class="voucher-title">${getDocumentTitle('contravoucher')}</div>

                    <div class="details-grid">
                        <div>
                            <div class="detail-item"><span class="detail-label">Voucher No:</span> ${selectedContra.voucherNumber}</div>
                            <div class="detail-item"><span class="detail-label">Date:</span> ${new Date(selectedContra.date).toLocaleDateString()}</div>
                        </div>
                        <div>
                            <div class="detail-item"><span class="detail-label">Manual Ref:</span> ${selectedContra.manualReceiptNo || '-'}</div>
                            <div class="detail-item"><span class="detail-label">Paid From (Source):</span> ${selectedContra.sourceAccount?.name || '-'}</div>
                        </div>
                    </div>
                    ...
                    <div class="total-section">
                        <div class="total-box">
                            <div class="total-row grand-total">
                                ...
                                <span>${formatCurrency(selectedContra.totalAmount)}</span>
                            </div>
                        </div>
                    </div>

                    <div style="margin-top: 30px;">
                        <p style="font-size: 14px; font-weight: bold; margin-bottom: 5px;">Voucher Narration:</p>
                        <p style="font-size: 14px; color: #555; background: #f9f9f9; padding: 10px; border-radius: 4px;">${selectedContra.mainNarration || 'No narration provided.'}</p>
                    </div>
                    ...
                    <div class="footer">
                        ...
                        <div class="signature-container">
                            <div class="signature-box">
                                ${selectedContra.signature ? `<img src="${selectedContra.signature}" class="signature-img" />` : ''}
                            </div>
    ...
    return (
        <div className="Contra-expenses-page">
            <div className="Contra-page-header">
                ...
                {hasPermission('create contra') && (
                    <button className="Contra-expenses-btn-dark" onClick={() => { resetForm(); setSelectedContra(null); setIsCreateOpen(true); }}>
            ...
            <div className="Contra-table-card">
                <div className="Contra-table-controls" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div className="Contra-modal-overlay">
                    <div className="Contra-modal-content Contra-large-modal">
                        ...
                        <div className="Contra-modal-body">
                            ...
                            {selectedContra.signature && (
                                <div className="Contra-view-signature mt-4">
                                    ...
                                    <div style={{ padding: '10px', border: '1px solid #e2e8f0', borderRadius: '8px', background: 'white', display: 'inline-block' }}>
                                        <img src={selectedContra.signature} alt="Signature" style={{ maxHeight: '80px' }} />
                                    </div>
                                </div>
                            )}
                        </div>
                        <div className="Contra-modal-footer">
                            <button className="Contra-btn-print" onClick={handlePrint} style={{ backgroundColor: '#1e293b', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div className="Contra-modal-overlay">
                    <div className="Contra-modal-content Contra-large-modal">
                        ...
                        <div className="Contra-modal-body">
                            ...
                            <div className="Contra-form-row">
                                ...
                                <div className="Contra-form-group Contra-half">
                                    ...
                                    >
                                        ...
                                        {accounts.filter(a => a.type === 'ASSETS' && !['Sundry Debtors', 'Accounts Receivable', 'Account Receivable'].includes(a.groupName) && !['Accounts Receivable', 'Account Receivable'].includes(a.name)).map(acc => (
                            ...
                            <div className="Contra-form-row mt-4">
                                <div className="Contra-form-group Contra-half">
                                    <label>Authorized Signature</label>
                                    <div className="signature-upload-wrapper" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                        <input type="file" accept="image/*" onChange={handleSignatureUpload} id="contra-sig-upload" style={{ display: 'none' }} />
                                        <label htmlFor="contra-sig-upload" style={{ background: '#f1f5f9', border: '2px dashed #cbd5e1', padding: '20px', borderRadius: '8px', textAlign: 'center', cursor: 'pointer' }}>
                                            {formData.signature ? (
